...
func emit_projectile(direction : Vector3, target : Vector3):
	...
	new_bullet.init(direction, target, projectile_speed,projectile_damage, projectile_lifetime)
	get_tree().root.get_child(2).add_child(new_bullet)
	new_bullet.global_position = business_end.global_position
	new_bullet.look_at(target, Vector3.UP, true)
